...
1.  **Strict Typisation**:
    ...
    - **Nullable Utility**: Use `Nullable<T>` (from `@/types/common.types`) for any variable or property that can be `null` or `undefined`. This makes the optionality of the value explicit and searchable.
...
3.  **Centralized Organization**:
    - All shared types must reside in `types/`.
    - Types should be separated semantically into modular files or sub-folders.
    - Local types (used only within a single file/component) are permissible but should be moved to `types/` if reused.
